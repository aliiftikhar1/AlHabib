...
export async function PUT(request,{params}) {
  ...
  try {
    ...
    const booking = await prisma.VisaBookings.findUnique({
    ...
    const newBooking = await prisma.VisaBookings.update({
      where:{booking_id: parseInt(id)},
      data: {
        status,
        updated_at: new Date(),
...
export async function DELETE(request,{params}) {
  ...
  console.log("id",id);
try {
  const bookings = await prisma.visaBookings.delete({where:{
